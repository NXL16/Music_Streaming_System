...
import { decryptChunk, importAesKey, hexToBytes } from "../mse/crypto";
import { fetchRange } from "../mse/fetcher";
import type { PreloadedSong, SegmentInfo, StreamMetadata } from "../mse/types";
...
export class ManagedMsePlayer {
  ...
  // ManagedMediaSource tells us when it is appropriate to fetch more media.
  // It starts false so a device can control network and battery use.
  ...
  async attach(
    ...
    this.mediaSource = new ManagedMediaSourceCtor();
    ...
    // ManagedMediaSource on iPhone needs remote playback disabled unless an
    // alternate AirPlay source is supplied. This POC deliberately disables it.
  ...
  detach(): void {
    ...
    if (this.audio) {
      ...
      this.audio.disableRemotePlayback = false;
  ...
  private onEndStreaming = (): void => {
    ...
    this.abortController?.abort();
    this.abortController = null;
    // Invalidate the in-flight loop. A later startstreaming event creates a
    // fresh controller and resumes buffering from the current playhead.
    this.fillGeneration++;
    this.downloading = false;
  };
