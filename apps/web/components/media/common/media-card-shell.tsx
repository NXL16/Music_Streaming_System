...
export default function MediaCardShell({
  ...
  metadataTextTertiary,
}: MediaCardShellProps) {
  const activeId = useMenuStore((s) => s.activeId);
  const isMenuOpen = activeId === id;
          ...
          isHero
            ? ({
                ...
                "--metadata-secondary-color":
                  metadataTextSecondary ?? metadataTextPrimary ?? "#ffffff",
                "--metadata-tertiary-color":
                  metadataTextTertiary ?? metadataTextPrimary ?? "#ffffff",
