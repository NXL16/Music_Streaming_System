...
const INSTRUMENTAL_LINE_CSS = `:host { width: 100%; display: grid; grid-template-columns: repeat(3, 10px); justify-content: start; column-gap: 5px; transform: scale(1); transform-origin: left center; } :host:has(span:dir(rtl)) { transform-origin: right center; } :host(.is-current:not(.ending)) { animation: var(--instrumental-enter-duration, 180ms) ease-out 0s 1 normal both instrumental-enter, 5s ease-in-out 0s infinite normal none running heartbeat; } :host(.is-current.ending) { animation: var(--instrumental-enter-duration, 180ms) ease-out 0s 1 normal both instrumental-enter, var(--instrumental-ending-animation-duration, 1s) ease-in 0s 1 normal forwards running heartbeat-end; } :host(.is-current) .dot { opacity: 0.3; } :host(.is-current) .dot--current { transition-property: opacity; opacity: 1; } .dot { width: 10px; height: 10px; display: inline-block; border-radius: 10px; background-color: var(--lyrics-dot-background-color, var(--systemPrimary)); } @keyframes instrumental-enter { 0% { opacity: 0; } 100% { opacity: 1; } } @keyframes heartbeat { 0% { transform: scale(1); } 50% { transform: scale(1.2); } 100% { transform: scale(1); } } @keyframes heartbeat-end { 0% { transform: scale(var(--instrumental-ending-start-scale, 1)); } 100% { transform: scale(1.4); } }`;
