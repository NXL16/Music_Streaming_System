...
const ACTIVE_LINE_TOP_RATIO = 0.2;
...
export class AmpLyricsDisplayTimeSynced extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!shadow.childNodes.length) {
      ...
      shadow.append(this.topSpacer, this.bottomSpacer, runtimeStyle);
  ...
  setActiveIndex(index: number, shouldScroll: boolean) {
    ...
    if (shouldScroll) this.scrollActiveLine();
  }
  ...
  private scrollActiveLine() {
    this.scrollLine(this.activeIndex);
  }

  private scrollLine(index: number) {
    ...
    this.scrollBy({
      top:
        lineRect.top - hostRect.top - this.clientHeight * ACTIVE_LINE_TOP_RATIO,
      behavior: "smooth",
    });
  }
  ...
  private updateSpacerHeights() {
    const topSpacerHeight = Math.max(
      0,
      this.clientHeight * ACTIVE_LINE_TOP_RATIO,
    );
    const bottomSpacerHeight = Math.max(
      0,
      this.clientHeight * (1 - ACTIVE_LINE_TOP_RATIO),
    );
