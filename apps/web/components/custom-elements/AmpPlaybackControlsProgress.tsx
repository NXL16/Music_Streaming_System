...
export default function AmpPlaybackControlsProgress({
  ...
  const AmpProgressTag = "amp-playback-controls-progress" as ElementType;

  return (
    <AmpProgressTag
      hydrated=""
      ref={containerRef}
      className={`${inDetailView ? "amp-playback-controls-progress" : `progress ${isProgressExpanded ? "progress--expanded" : "progress--compact"}`}`}
    />
  );
}
