...
export const PERSONALIZATION_MODEL_VERSION = 4;
const MIN_HISTORY = 3;
...
type ShelfIntent = 'top' | 'affinity' | 'fresh' | 'discover';
type SnapshotSelect = {
  resourceId: true;
  resourceType: true;
  artistName: true;
...
export class RecommendationEngineService {
  ...
  async generateUserShelves(userId: string): Promise<GeneratedShelf[]> {
    ...
    const ranked = this.rank(
      await this.collectCandidates(userId, profile),
      profile,
    );
    ...
    await addShelf(
      'user-top-picks',
      'Top Picks for You',
      this.rankForShelf(ranked, 'top'),
    );
    await addShelf(
      'user-because-you-like',
      'Because You Like This Sound',
      this.rankForShelf(
        ranked.filter((x) => x.sources.has('affinity')),
        'affinity',
      ),
    );
    await addShelf(
      ...
      'Fresh For You',
      this.rankForShelf(
        ranked.filter((x) => x.sources.has('fresh')),
        'fresh',
      ),
    );
    await addShelf(
  ...
  private async collectCandidates(
    ...
    if (genres.length)
      add(
        await this.prisma.recommendationResourceSnapshot.findMany({
          where: {
            resourceType: 'songs',
            resourceId: { notIn: listenedIds },
            genreNames: { hasSome: genres },
          },
    ...
    add(
      await this.prisma.recommendationResourceSnapshot.findMany({
        where: {
          ...
          ...(genres.length ? { genreNames: { hasSome: genres } } : {}),
        },
        orderBy: [{ releaseDate: 'desc' }, { resourceId: 'asc' }],
        take: CANDIDATE_LIMIT,
        select,
    ...
    const [collaborative, popular] = await Promise.all([
      this.collaborativeCandidates(userId, listenedIds, select),
      this.popularCandidates(listenedIds, select),
    ]);
    add(collaborative.rows, 'collaborative');
    add(popular.rows, 'popular');
    for (const [id, score] of collaborative.scores) {
      const candidate = candidates.get(id);
      if (candidate) candidate.collaborative = score;
    }
    for (const [id, score] of popular.scores) {
      const candidate = candidates.get(id);
      if (candidate) candidate.popularity = score;
    }
    return [...candidates.values()];
  }

  private async collaborativeCandidates(
    userId: string,
    listenedIds: string[],
    select: SnapshotSelect,
  ) {
    if (!listenedIds.length)
      return { rows: [], scores: new Map<string, number>() };
    const overlaps = await this.prisma.userListeningStats.findMany({
      where: {
        userId: { not: userId },
        songId: { in: listenedIds },
        playCount: { gt: 0 },
      },
      select: {
        userId: true,
    ...
    const top = [...raw.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, CANDIDATE_LIMIT);
    const maximum = top[0]?.[1] ?? 1;
    const scores = new Map(top.map(([id, score]) => [id, score / maximum]));
    const rows = scores.size
      ? await this.prisma.recommendationResourceSnapshot.findMany({
          where: {
            resourceType: 'songs',
            resourceId: { in: [...scores.keys()] },
          },
          select,
        })
      : [];
  ...
  private async popularCandidates(
    listenedIds: string[],
    select: SnapshotSelect,
  ) {
    const grouped = await this.prisma.userListeningStats.groupBy({
      by: ['songId'],
      where: { songId: { notIn: listenedIds }, playCount: { gt: 0 } },
    ...
    const scores = new Map(
      grouped.map((row) => {
        ...
        const quality = Math.max(
          0.15,
          1 +
            ((row._sum.completionCount ?? 0) - (row._sum.skipCount ?? 0)) /
              Math.max(1, plays),
        );
        return [row.songId, (plays / maximum) * quality];
      }),
    );
    const rows = scores.size
      ? await this.prisma.recommendationResourceSnapshot.findMany({
  ...
  private rank(candidates: Candidate[], profile: TasteProfile): Candidate[] {
    const artists = new Map(profile.artists.map((x) => [x.name, x.weight]));
    const genres = new Map(profile.genres.map((x) => [x.name, x.weight]));
    for (const candidate of candidates) {
      candidate.artistAffinity = artists.get(candidate.artistName) ?? 0;
      candidate.genreAffinity = Math.min(
        1,
        candidate.genreNames.reduce(
          (sum, name) => sum + (genres.get(name) ?? 0),
          0,
        ),
      );
      candidate.novelty = candidate.artistAffinity === 0 ? 1 : 0;
      candidate.score =
        candidate.genreAffinity * 0.3 +
        candidate.artistAffinity * 0.2 +
        candidate.collaborative * 0.2 +
        candidate.popularity * 0.12 +
        this.freshness(candidate.releaseDate) * 0.1 +
  ...
  private rankForShelf(
    candidates: Candidate[],
    intent: ShelfIntent,
  ): Candidate[] {
    const scoreFor = (candidate: Candidate) => {
      const freshness = this.freshness(candidate.releaseDate);
      switch (intent) {
        case 'affinity':
          return (
            candidate.genreAffinity * 0.45 +
            candidate.artistAffinity * 0.25 +
            candidate.collaborative * 0.2 +
            freshness * 0.1
          );
        case 'fresh':
        ...
        case 'discover':
          return (
            ...
            candidate.collaborative * 0.25 +
            freshness * 0.12 +
            candidate.popularity * 0.08
          );
        default:
  ...
  private freshness(value: string): number {
    const timestamp = new Date(value).getTime();
    return Number.isFinite(timestamp)
      ? Math.max(0, 1 - (Date.now() - timestamp) / (180 * 86_400_000))
      : 0;
  }
}
